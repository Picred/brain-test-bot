...
def test_prossima_domanda(mocker: MockerFixture) -> None:
    ...
    with patch('src.handlers.domanda.len', return_value=5):
        with patch('src.handlers.domanda.domanda') as mocked_domanda:
...
def test1_prossima_domanda(mocker: MockerFixture) -> None:
    ...
    with patch('src.handlers.domanda.len', return_value=len_data):
        with patch('src.handlers.domanda.genera_commento') as mocked_genera_commento:
            prossima_domanda(update,context,data)
